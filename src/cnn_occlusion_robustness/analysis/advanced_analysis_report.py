...
class OcclusionRobustnessAnalyzer:
    ...
    def load_evaluation_data(self) -> pd.DataFrame:
        ...
        if not csv_files:
            raise FileNotFoundError(f"No evaluation matrix files found in {self.results_dir}")
    ...
    def load_training_data(self) -> Dict:
        ...
        json_files = glob.glob(os.path.join(self.results_dir, "training_results_*.json"))
    ...
    def create_publication_heatmap(self):
        ...
        # Improve formatting
        ax.set_title('Cross-Domain Performance Matrix:\nTraining Condition vs. Test Condition', 
        ...
        plt.savefig(os.path.join(self.output_dir, 'figures', 'performance_heatmap.pdf'), 
                    bbox_inches='tight')
        plt.close()
    ...
    def create_robustness_comparison(self):
        ...
        fig.suptitle('Robustness Analysis Across Training Conditions', fontsize=16, fontweight='bold')

        # Calculate robustness metrics
        robustness_data = self.calculate_robustness_metrics()

        # 1. Clean vs Average Noisy Performance
        axes[0,0].scatter(robustness_data['clean_accuracy'], 
                          robustness_data['mean_noisy_accuracy'],
                          s=100, alpha=0.7, c=range(len(robustness_data)), cmap='viridis')
        ...
        for i, model in enumerate(robustness_data.index):
            axes[0,0].annotate(model.split('_')[0], 
        ...
        robustness_data['robustness_gap'].plot(kind='bar', ax=axes[0,1], color='skyblue')
        ...
        robustness_data['worst_case_accuracy'].plot(kind='bar', ax=axes[0,2], color='lightcoral')
        ...
        axes[1,1].legend(title='Training Condition', bbox_to_anchor=(1.05, 1), loc='upper left')
        ...
        if self.training_data:
            ...
            axes[1,2].scatter(training_efficiency['final_val_acc'], 
            ...
            axes[1,2].text(0.5, 0.5, 'Training data\nnot available', 
                           ha='center', va='center', transform=axes[1,2].transAxes)

        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'figures', 'robustness_analysis.png'), 
                    dpi=300, bbox_inches='tight')
        plt.savefig(os.path.join(self.output_dir, 'figures', 'robustness_analysis.pdf'), 
                    bbox_inches='tight')
        plt.close()
    ...
    def calculate_robustness_metrics(self) -> pd.DataFrame:
        ...
        for model in self.evaluation_data.index:
            ...
            noisy_accs = [row[col] for col in row.index if col != 'none' and not pd.isna(row[col])]
            ...
            if noisy_accs:
                metrics[model] = {
                    ...
                    'robustness_gap': clean_acc - np.mean(noisy_accs) if not pd.isna(clean_acc) else np.nan,
                    'worst_case_gap': clean_acc - np.min(noisy_accs) if not pd.isna(clean_acc) else np.nan,
                    'relative_robustness': np.mean(noisy_accs) / clean_acc if not pd.isna(clean_acc) and clean_acc > 0 else np.nan
                }
        ...
        for model in self.evaluation_data.index:
            ...
            for category, effects in self.effect_categories.items():
                ...
                if available_effects:
                    accs = [row[eff] for eff in available_effects if not pd.isna(row[eff])]
    ...
    def create_category_analysis(self):
        ...
        fig.suptitle('Performance Analysis by Effect Category', fontsize=16, fontweight='bold')

        category_perf = self.calculate_category_performance()

        # 1. Category comparison heatmap
        sns.heatmap(category_perf.T, annot=True, fmt='.3f', cmap='RdYlGn', 
        ...
        for model in self.evaluation_data.index:
            ...
            if train_effect in row.index:
                ...
                cross_effects = [col for col in row.index if col != train_effect and not pd.isna(row[col])]
                if cross_effects:
                    cross_domain_scores.append(np.mean([row[col] for col in cross_effects]))
                else:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'figures', 'category_analysis.png'), 
                    dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def calculate_training_benefit(self) -> pd.Series:
        ...
        for effect in self.evaluation_data.columns:
            if effect != 'none':
                ...
                specialized_models = [model for model in self.evaluation_data.index 
                                      if model == effect]

                if specialized_models:
                    specialized_perf = self.evaluation_data.loc[specialized_models[0], effect]
    ...
    def create_significance_matrix(self):
                ...
                if i != j:
                    ...
                    if len(common_conditions) > 1:
                        ...
                        if len(p1_common) > 1:
                            ...
                            effect_sizes[i, j] = np.mean(diff) / np.std(diff) if np.std(diff) > 0 else 0
        ...
        with open(os.path.join(self.output_dir, 'statistical_tests', 'significance_results.json'), 'w') as f:
    ...
    def create_training_dynamics_figure(self):
        ...
        for model_name, data in self.training_data.items():
            if 'history' in data:
                ...
                axes[0,0].plot(epochs, history['train_acc'], label=model_name, alpha=0.8)
                axes[0,1].plot(epochs, history['val_acc'], label=model_name, alpha=0.8)
                axes[1,0].plot(epochs, history['train_loss'], label=model_name, alpha=0.8)
                axes[1,1].plot(epochs, history['val_loss'], label=model_name, alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'figures', 'training_dynamics.png'), 
                    dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _write_executive_summary(self, f):
        ...
        f.write(f"- **Best Overall Model:** {best_overall} (Mean noisy accuracy: {best_score:.3f})\n")

        # Most robust model
        most_robust = robustness_metrics['robustness_gap'].idxmin()
        robustness_gap = robustness_metrics.loc[most_robust, 'robustness_gap']

        f.write(f"- **Most Robust Model:** {most_robust} (Robustness gap: {robustness_gap:.3f})\n")
        ...
        if not effect_severity.empty:
            ...
            f.write(f"- **Most Challenging Effect:** {worst_effect} (Performance drop: {worst_drop:.3f})\n")
        ...
        if not training_benefit.empty:
            ...
            f.write(f"- **Highest Training Benefit:** {best_benefit} (Improvement: {benefit_value:.3f})\n")

        f.write("\n### Performance Summary\n\n")
        f.write(f"- **Models Evaluated:** {len(self.evaluation_data.index)}\n")
        f.write(f"- **Test Conditions:** {len(self.evaluation_data.columns)}\n")
        f.write(f"- **Mean Clean Performance:** {robustness_metrics['clean_accuracy'].mean():.3f}\n")
        f.write(f"- **Mean Noisy Performance:** {robustness_metrics['mean_noisy_accuracy'].mean():.3f}\n")
        f.write(f"- **Average Robustness Gap:** {robustness_metrics['robustness_gap'].mean():.3f}\n")
    ...
    def _write_detailed_findings(self, f):
        ...
        for model in self.evaluation_data.index:
            ...
            if train_effect in row.index:
                ...
                cross_effects = [col for col in row.index if col != train_effect and not pd.isna(row[col])]
                if cross_effects:
                    cross_domain_perfs.append(np.mean([row[col] for col in cross_effects]))
        ...
        for category in self.effect_categories.keys():
            if category in category_perf.columns:
                ...
                f.write(f"- **{category.title()} Effects:** Mean accuracy {mean_perf:.3f}\n")
    ...
    def _write_statistical_analysis(self, f):
        ...
        f.write("Paired t-tests were performed between all model pairs across common test conditions.\n\n")

        # Load significance results if available
        sig_file = os.path.join(self.output_dir, 'statistical_tests', 'significance_results.json')
        if os.path.exists(sig_file):
            ...
            significant_pairs = np.sum(p_values < 0.05) - len(models)  # Exclude diagonal
            total_pairs = len(models) * (len(models) - 1)

            f.write(f"- **Significant differences found:** {significant_pairs}/{total_pairs} model pairs (p < 0.05)\n")

            # Large effect sizes
            large_effects = np.sum(np.abs(effect_sizes) > 0.8) - len(models)
            f.write(f"- **Large effect sizes:** {large_effects}/{total_pairs} comparisons (|d| > 0.8)\n\n")
    ...
    def _write_recommendations(self, f):
        ...
        # Best overall model
        best_overall = robustness_metrics['mean_noisy_accuracy'].idxmax()
        f.write(f"1. **For general deployment:** Use {best_overall} for best overall performance across conditions.\n")

        # Most robust model
        most_robust = robustness_metrics['robustness_gap'].idxmin()
        f.write(f"2. **For unknown conditions:** Use {most_robust} for most consistent performance.\n")

        # Clean performance model
        best_clean = robustness_metrics['clean_accuracy'].idxmax()
        f.write(f"3. **For clean conditions:** Use {best_clean} for optimal clean performance.\n\n")
        ...
        if not training_benefit.empty:
            ...
            if len(beneficial_effects) > 0:
                ...
                f.write("- Specialized training shows minimal benefit over clean training.\n")

        f.write("\n### Deployment Considerations\n\n")
        f.write("1. **Environment Assessment:** Evaluate expected occlusion types in deployment environment.\n")
        f.write("2. **Performance Monitoring:** Implement performance monitoring to detect distribution shifts.\n")
        f.write("3. **Ensemble Methods:** Consider ensemble approaches for critical applications.\n")
        f.write("4. **Data Augmentation:** Use appropriate augmentation during training for robustness.\n")
    ...
    def _write_appendix(self, f):
        ...
        f.write("- **Consistency:** Standard deviation of performance across noisy conditions\n")
        f.write("- **Training Benefit:** Specialized model accuracy - Clean model accuracy on same condition\n")
    ...
    def save_summary_tables(self):
        ...
        # Robustness metrics
        robustness_metrics = self.calculate_robustness_metrics()
        robustness_metrics.to_csv(os.path.join(self.output_dir, 'tables', 'robustness_metrics.csv'))

        # Category performance
        category_perf = self.calculate_category_performance()
        category_perf.to_csv(os.path.join(self.output_dir, 'tables', 'category_performance.csv'))

        # Effect severity
        effect_severity = self.calculate_effect_severity()
        effect_severity.to_csv(os.path.join(self.output_dir, 'tables', 'effect_severity.csv'))

        # Training benefit
        training_benefit = self.calculate_training_benefit()
        training_benefit.to_csv(os.path.join(self.output_dir, 'tables', 'training_benefit.csv'))
...
def main():
    parser = argparse.ArgumentParser(description="Advanced Analysis of Occlusion Robustness Results")
